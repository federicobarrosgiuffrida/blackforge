...
| Prestazioni backend CUDA (pool di memoria, attention batchizzata) | ✅ Pool di memoria device per-device (elimina `cudaMalloc`/`cudaFree` per-operazione) e attention ricalcolata con kernel batchizzati (`attention_batched.hpp`/`.cu`, un lancio per TUTTE le combinazioni batch×testa invece di uno per ciascuna) — insieme al Tensor Core esteso sopra, portano un training reale (~23M parametri, `batch_size 8`) da ore stimate a circa 1 ora: misurato un fattore ~7-8× sul tempo per step (150 step da 73,9s a 9,5s). Nessun cambiamento alla matematica: la suite CUDA (391 test) resta verde ad ogni passo |
...
15. ✅ Prestazioni del backend CUDA: un training reale (~23M parametri, `TinyStories`, `batch_size 8`) risultava stimato in **ore**. Tre interventi, ciascuno verificato con l'intera suite prima del successivo: **pool di memoria device per-device** (elimina `cudaMalloc`/`cudaFree` per-operazione, guadagno modesto da solo, ~20%), **attention batchizzata** (`attention_batched.hpp`/`.cu`, un kernel per TUTTE le combinazioni batch×testa invece di uno per ciascuna — 64 lanci → pochi fissi per layer con `batch_size=8`/8 teste — guadagno anch'esso modesto, ~20-25%: il numero di lanci non era il vero collo di bottiglia), e **Tensor Core BF16 esteso ad attention/feedforward** (le proiezioni Q/K/V/Out/W1/W2, prima ferme a SGEMM FP32 — il guadagno decisivo, conferma che il calcolo stesso era il collo di bottiglia dominante). Misurato un fattore **~7-8×** sul tempo per step (150 step a `batch_size 8`: da 73,9s a 9,5s). Nessun cambiamento alla matematica: la suite CUDA (391 test) resta verde ad ogni passo, inclusi gradient checking e parità CPU/GPU per attention/feedforward.
